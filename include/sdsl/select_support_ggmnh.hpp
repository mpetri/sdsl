...
*/
/*! \file select_support_ggmnh.hpp
    \brief select_support_ggmnh.hpp contains classes that support a sdsl::bit_vector with log(n) select operations.
	\author Matthias Petri
*/
#ifndef INCLUDED_SDSL_select_support_ggmnh
#define INCLUDED_SDSL_select_support_ggmnh
 ...
 */
class select_support_ggmnh : public select_support
{
    public:
    ...
    public:
        explicit select_support_ggmnh(const int_vector<1>* v=NULL);
        select_support_ggmnh(const select_support_ggmnh& rs);
        ~select_support_ggmnh();
         ...
         */
        select_support_ggmnh& operator=(const select_support_ggmnh& ss);
        //! swap Operator
        /*! Swap two select_support_ggmnh in constant time.
            Required for the Container Concept of the STL.
         */
        void swap(select_support_ggmnh& ss);
...
inline select_support_ggmnh& select_support_ggmnh::operator=(const select_support_ggmnh& ss)
{
    if (this != &ss) {
        set_vector(ss.m_v);
        m_superblockrank = ss.m_superblockrank;
...
inline void select_support_ggmnh::swap(select_support_ggmnh& ss)
{
...
inline void select_support_ggmnh::init(const int_vector<1>* v)
{
    set_vector(v);
    if (m_v == NULL) return;
    if (m_v->empty()) {
...
inline const select_support_ggmnh::size_type select_support_ggmnh::select(size_type idx) const
{
...
inline const select_support_ggmnh::size_type select_support_ggmnh::operator()(size_type idx) const
{
    return select(idx);
}

inline select_support_ggmnh::size_type select_support_ggmnh::serialize(std::ostream& out, structure_tree_node* v, std::string name)const
{
    ...
    written_bytes += m_rank_samples.serialize(out, child, "rank samples");
    structure_tree::add_size(child, written_bytes);
    return written_bytes;
}

inline void select_support_ggmnh::load(std::istream& in, const int_vector<1>* v)
{
    set_vector(v);
...
inline select_support_ggmnh::~select_support_ggmnh()
{
}

inline void select_support_ggmnh::set_vector(const int_vector<1>* v)
{
    if (v != NULL) {
        m_v = v;
