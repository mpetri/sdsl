...
*/
/*! \file select_support_vs_s7.hpp
    \brief select_support_vs_s7.hpp contains classes that support a sdsl::bit_vector with constant time select information.
	\author Simon Gog
*/
#ifndef INCLUDED_SDSL_select_support_vs_s7
#define INCLUDED_SDSL_select_support_vs_s7
...
#ifdef SDSL_DEBUG_select_support_vs_s7
...
struct select_support_vs_s7_trait {
...
template<>
struct select_support_vs_s7_trait<0,1> {
    typedef select_support::size_type	size_type;
...
template<>
struct select_support_vs_s7_trait<1,1> {
    typedef select_support::size_type	size_type;
...
template<>
struct select_support_vs_s7_trait<10,2> {
    typedef select_support::size_type	size_type;
...
template<>
struct select_support_vs_s7_trait<01,2> {
    typedef select_support::size_type	size_type;
...
class select_support_vs_s7 : public select_support
{
    public:
    ...
    public:
        ...
        void copy(const select_support_vs_s7<b, pattern_len>& ss);
    public:
        explicit select_support_vs_s7(const int_vector<1>* v=NULL);
        select_support_vs_s7(const select_support_vs_s7<b,pattern_len>& ss);
        ~select_support_vs_s7();
        ...
        select_support_vs_s7<b, pattern_len>& operator=(const select_support_vs_s7& ss);

        //! Swap operator
        /*! This swap operator swaps two select_support_vs_s7s in constant time.
         */
        void swap(select_support_vs_s7<b, pattern_len>& ss);

};


template<uint8_t b, uint8_t pattern_len>
select_support_vs_s7<b,pattern_len>::select_support_vs_s7(const int_vector<1>* f_v):select_support(f_v)
{
    init(f_v);
}

template<uint8_t b, uint8_t pattern_len>
select_support_vs_s7<b,pattern_len>::select_support_vs_s7(const select_support_vs_s7& ss):select_support(ss.m_v)
{
    copy(ss);
}

template<uint8_t b, uint8_t pattern_len>
select_support_vs_s7<b, pattern_len>& select_support_vs_s7<b,pattern_len>::operator=(const select_support_vs_s7& ss)
{
    if (this != &ss) {
        copy(ss);
    }
...
template<uint8_t b, uint8_t pattern_len>
void select_support_vs_s7<b,pattern_len>::swap(select_support_vs_s7& ss)
{
...
template<uint8_t b, uint8_t pattern_len>
void select_support_vs_s7<b,pattern_len>::copy(const select_support_vs_s7<b, pattern_len>& ss)
{
...
template<uint8_t b, uint8_t pattern_len>
select_support_vs_s7<b,pattern_len>::~select_support_vs_s7()
{
}

template<uint8_t b, uint8_t pattern_len>
void select_support_vs_s7<b,pattern_len>::init(const int_vector<1>* v)
{
...
/*    fprintf(stdout, "B[] = ");
    ...
    num_ones = select_support_vs_s7_trait<b,pattern_len>::arg_cnt(*v);

    ones_per_inventory = num_bits == 0 ? 0 : ( num_ones * MAX_ONES_PER_INVENTORY + num_bits - 1 ) / num_bits;
            ...
            for( int j = 0; j < 64; j++ ) {
                ...
                if ( bits[ i ] & 1ULL << j ) {
                    if ( ( d & ones_per_inventory_mask ) == 0 ) {
                        ...
                        ones = min( num_ones - d, (uint64_t)ones_per_inventory );

                        // We must always count (possibly unused) diff16's. And we cannot store less then 4 diff16.
                        diff16 += max( 4, ( ones + ones_per_sub16 - 1 ) >> log2_ones_per_sub16 );
...
template<uint8_t b, uint8_t pattern_len>
inline const typename select_support_vs_s7<b,pattern_len>::size_type select_support_vs_s7<b,pattern_len>::select(size_type rank)const
{
...
template<uint8_t b, uint8_t pattern_len>
inline const typename select_support_vs_s7<b,pattern_len>::size_type select_support_vs_s7<b,pattern_len>::operator()(size_type i)const
{
    return select(i);
}

template<uint8_t b, uint8_t pattern_len>
void select_support_vs_s7<b,pattern_len>::set_vector(const int_vector<1>* v)
{
    m_v = v;
}

template<uint8_t b, uint8_t pattern_len>
typename select_support_vs_s7<b,pattern_len>::size_type select_support_vs_s7<b,pattern_len>::serialize(std::ostream& out, structure_tree_node* v, std::string name)const
{
...
template<uint8_t b, uint8_t pattern_len>
void select_support_vs_s7<b,pattern_len>::load(std::istream& in, const int_vector<1>* v)
{
    set_vector(v);
